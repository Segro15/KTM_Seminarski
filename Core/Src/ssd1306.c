...
{
    const uint8_t *glyph = font6x8[digit];
...
void OLED_DrawAngle(int angle)
{
    ...
    // Očisti zadnju liniju (page 7)
    OLED_SetPosition(7, 0);
    for (int i = 0; i < 30; i++)
        OLED_Data(0x00);

    uint8_t col = 0; // centrirano

    if (hundreds > 0) {
        OLED_DrawChar(7, col, hundreds);
        col += 7;
    }
    if (hundreds > 0 || tens >0) {
    	OLED_DrawChar(7, col, tens);
    	col = col + 7;
    }

    OLED_DrawChar(7, col, ones);
    col += 7;
